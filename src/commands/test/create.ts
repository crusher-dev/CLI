...
program.option("-p, --port <number>", "port number").option("-t, --token <string>", "Crusher user token").parse(process.argv);
...
export default class CommandBase {
	...
	async createTest(flags) {
		...
		if (process.platform === "darwin") {
			execSync(
				`${resolvePathToAppDirectory('bin/"Crusher Recorder.app"/Contents/MacOS/"Crusher Recorder"')} --no-sandbox --exit-on-save --projectId=${
			...
			execSync(
				`${resolvePathToAppDirectory("bin/electron-app")} --no-sandbox --exit-on-save --projectId=${projectConfig.project} --token=${userInfo?.token}`,
				{ stdio: "ignore" },
			);
		}
