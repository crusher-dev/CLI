...
export default class CommandBase {
	...
	async run(): Promise<any> {
		...
		if (!projectConfig || !projectConfig.project) {
			throw new Error("Crusher not initialized in this project. Run `crusher-cli init` to fix this.");
		}
		...
		if (res.method === 0) {
			...
			console.log("Your Invite link:", await getInviteLink(projectConfig.project));
		}
	}
}
