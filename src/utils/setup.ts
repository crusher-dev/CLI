...
async function installMacBuild(isUpgrading) {
	...
	if (isUpgrading) {
		...
		cli.info("Crusher recorder not installed. Downloading and installing it.\n");
	}
...
async function installLinuxBuild(isUpgrading) {
	...
	await cli.action.start("Unzipping");
	execSync(`cd ${path.dirname(recorderZipPath)} && unzip -o ${path.basename(recorderZipPath)} -d . && rm -R ${path.basename(recorderZipPath)}`);
...
export async function installCrusherRecorder() {
	...
	if (process.platform === "darwin") {
		if (fs.existsSync(resolvePathToAppDirectory("bin/Crusher Recorder.app")) && isInstallingBuild == false) {
			return;
		}

		await installMacBuild(isUpgrading);
	} else if (process.platform === "linux") {
		if (fs.existsSync(resolvePathToAppDirectory("bin/electron-app")) && isInstallingBuild == false) {
			return;
		}
