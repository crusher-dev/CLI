...
const runTests = async (host: string | undefined) => {
	...
	try {
		...
		const res = await axios.post(
			resolveBackendServerUrl(`/projects/${projectConifg.project}/tests/actions/run`),
			{
				host: host,
				context: context,
			},
		...
		await new Promise((resolve) => {
			...
			const poll = setInterval(async () => {
				...
				if (buildInfo.status === "PASSED" || buildInfo.status === "FAILED" || buildInfo.status === "MANUAL_REVIEW_REQUIRED") {
					clearInterval(poll);
					await cli.action.stop(
						buildInfo.status === "PASSED" ? `Build passed in ${parseInt(buildInfo.duration)}s` : `Build failed in ${parseInt(buildInfo.duration)}s`,
					);
