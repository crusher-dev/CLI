...
export const getGitLastCommitSHA = () =>
	new Promise((resolve, reject) => {
		const cmd = getRuntimeEnv().GITHUB_HEAD_REF ? `git ls-remote origin ${getRuntimeEnv().GITHUB_HEAD_REF}` : "git rev-parse HEAD";
...
export const getGitBranchName = () =>
	new Promise((resolve, reject) => {
		...
		const headRef = getRuntimeEnv().GITHUB_HEAD_REF ? (getRuntimeEnv().GITHUB_HEAD_REF as any) : null;
		if (headRef) {
			resolve(headRef);
		} else {
			exec(
