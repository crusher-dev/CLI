...
async function installNSetupOnMac() {
	...
	execSync(
		`cd ${path.dirname(recorderZipPath)} && tar -xvzf ${path.basename(recorderZipPath)} && rm -R ${path.basename(
			recorderZipPath,
		...
		return new Promise(async (resolve, rej) => {
			...
			const tunnelPromises = data.map(({ name, url }) => {
				return new Promise((res, rej) => {
					var spann;
					try {
						spann = spawn(resolvePathToAppDirectory(`bin/cloudflared`), [`tunnel`, `--url`, `${url}`]);

						console.log("URL", resolvePathToAppDirectory(`bin/cloudflared`), [`tunnel --url ${url}`]);
					} catch (e) {
						console.log("error", e);
					}
